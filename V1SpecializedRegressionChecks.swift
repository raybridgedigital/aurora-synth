...
@main struct AuroraV1SpecializedRegressionChecks {
    @MainActor static func main() {
        ...
        let folder=FileManager.default.temporaryDirectory.appendingPathComponent("AuroraV1Specialized-"+UUID().uuidString)
        ...
        for sound in FactoryBank.all {
            ...
            let decoded = try! JSONDecoder().decode(SoundPreset.self, from: encoded)
            precondition(SynthModel.sanitized(decoded) != nil)
            precondition(decoded.layers == sound.layers && decoded.globals == sound.globals && decoded.macros == sound.macros)
            ...
            precondition(decoded.soundMatrix == sound.soundMatrix && decoded.performanceMatrix == sound.performanceMatrix && decoded.importedWavetables == sound.importedWavetables)

            for index in 0..<8 {
                model.patch = sound
                model.macro(index, 0)
                precondition(abs(model.patch.macros[index]) < 0.000001)
                precondition(SynthModel.sanitized(model.patch) != nil)

                model.patch = sound
                model.macro(index, 1)
                precondition(abs(model.patch.macros[index] - 1) < 0.000001)
                precondition(SynthModel.sanitized(model.patch) != nil)
            }

            let settings = sound.xy ?? XYSettings()
            precondition(settings.valid)
            model.patch = sound
            model.moveXY(x: 1, y: 0)
            precondition(abs(model.patch.macros[settings.x.macro] - settings.x.value(1)) < 0.000001)
        ...
        model.loadPreset(legacy, panic: false);precondition((model.patch.phaserMix ?? 0)==0 && model.patch.layers[0][33]==0)
        ...
        // Current contract: delay/shimmer remain patch FX; EQ 20...22 is session-sticky and is not patch data.
        ...
        model.global(23,0.35);model.global(24,7);model.global(28,0.6);model.global(32,1);model.global(36,5)
        model.saveCurrent();model.persist()
        let fx208=try! JSONDecoder().decode(SavedSession.self,from:Data(contentsOf:folder.appendingPathComponent("session.json")))
        precondition(fx208.patch.globalValue(16)==0 && fx208.patch.globalValue(17)==500)
        precondition(fx208.patch.globalValue(20)==0 && fx208.patch.globalValue(21)==0 && fx208.patch.globalValue(22)==0)
        ...
        print("PASS: Aurora factory sounds load under the current schema; macro endpoints, configurable XY behavior, motion and save round trips pass.")
        ...
        model.loadPreset(legacy, panic: false);precondition(model.patch.layers[0][44]==0 && model.patch.layers[0][51]==0)
        ...
        model.loadPreset(portable, panic: false);model.set(0,1,2);precondition(model.patch.layers[0][44]==0)
        ...
        restored.loadPreset(legacy, panic: false);precondition(!restored.motionSettings.enabled)
        ...
        toolsModel.copyLayer(0);let beforePaste=toolsModel.patch.layers[1];toolsModel.pasteLayer(1, panic: false)
